...
namespace chil::app
{
	...
	int Run(win::IWindow& window)
	{
		...
		//cursor triangle
		ComPtr<ID3D12Resource> cursorVertexBuffer;
		UINT ncursorVertices;
		{
			// the content data 
			...
			{
				const CD3DX12_HEAP_PROPERTIES heapProps{ D3D12_HEAP_TYPE_DEFAULT };
				const auto resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(sizeof(cursorVertexData));
				device->CreateCommittedResource(
					&heapProps,
					D3D12_HEAP_FLAG_NONE,
					&resourceDesc,
					D3D12_RESOURCE_STATE_COPY_DEST,
					nullptr, IID_PPV_ARGS(&cursorVertexBuffer)
				) >> chk;
			}
			// create committed resource for cpu upload of vertex data 
			ComPtr<ID3D12Resource> vertexUploadBuffer;
			...
			// copy array of vertex data to upload buffer 
			{
				Vertex* mappedVertexData = nullptr;
				vertexUploadBuffer->Map(0, nullptr, reinterpret_cast<void**>(&mappedVertexData)) >> chk;
				rn::copy(cursorVertexData, mappedVertexData);
				vertexUploadBuffer->Unmap(0, nullptr);
			}
			// reset command list and allocator  
			commandAllocator->Reset() >> chk;
			commandList->Reset(commandAllocator.Get(), nullptr) >> chk;
			// copy upload buffer to vertex buffer 
			commandList->CopyResource(cursorVertexBuffer.Get(), vertexUploadBuffer.Get());
			// transition vertex buffer to vertex buffer state 
			{
				const auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(
					cursorVertexBuffer.Get(),
					D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
				commandList->ResourceBarrier(1, &barrier);
			}
			// close command list  
		...
		// Create the vertex buffer view. 
		const D3D12_VERTEX_BUFFER_VIEW cursorVertexBufferView{
			.BufferLocation = cursorVertexBuffer->GetGPUVirtualAddress(),
		...
		ComPtr<ID3D12Resource> cubeFaceTexture;
		{
			...
			DirectX::LoadFromWICFile(L"cube_face.jpeg", DirectX::WIC_FLAGS_NONE, nullptr, image) ;
		...
		//create the curosr texture
		ComPtr<ID3D12Resource> cursorTexture;
		{
			// load image data from disk 
			DirectX::ScratchImage image;
			...
			const auto subresourceData = vi::iota(0, (int)mipChain.GetImageCount()) |
				...
				rn::to<std::vector>();
					...
					ComPtr<ID3D12Resource> uploadBuffer;
					{
						const CD3DX12_HEAP_PROPERTIES heapProps{ D3D12_HEAP_TYPE_UPLOAD };
						const auto uploadBufferSize = GetRequiredIntermediateSize(
							cursorTexture.Get(), 0, (UINT)subresourceData.size()
						);
						const auto resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(uploadBufferSize);
						device->CreateCommittedResource(
							&heapProps,
		...
		ComPtr<ID3D12PipelineState> pipelineState;
		{
			...
			device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineState)) >> chk;
		}
